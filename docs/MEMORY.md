...
## Situation Cards (Finalized)

11 unique situation cards: 5 batting, 5 bowling, 1 shared (Old School Cricket Only).

### Batting Situation Cards
1. **DRS Review** — if batsman gets out, overturn to dot ball (wicket insurance)
2. **Power Surge** — all outcomes upgrade one tier (dot→1, 1→2, 2→4, 4→6). Weaknesses still trigger.
3. **Retired Out** — discard played batsman, play a replacement from hand before delivery resolves
4. **Switch Hit** — mirror the batsman's zones (off↔leg, 5th stump→leg). Middle stays. Fielding not mirrored.
5. **Trot Down** — batsman charges the bowler. Good length→Full, Short→Good length, Full→Full.

### Bowling Situation Cards
1. **Mankad** — opponent must discard played batsman and play a different one. If none available, all outcomes downgrade one tier.
2. **Review Appeal** — if result is a dot ball, 40% chance it becomes a wicket.
3. **Cramps** — discard played bowler, play a replacement from hand before delivery resolves.
4. **Invariable Bounce** — all batter outcomes downgrade one tier. Stacks with adjective downgrade.
5. **Day 5 Pitch** — bowler's line shifts one away from body (leg→mid, mid→off, off→5th, 5th→wide). Length unchanged.

### Shared
6. **Old School Cricket Only** — cancels opponent's situation card. If both play it, both cancelled. Available in both batting and bowling decks.

Full details and resolution order in `docs/situation-cards.md`.
