...
function main(): void {
  ...
  // Currently: 2 nations expanded (India, Australia) → 13×2 + 11×10 + 5 = 141 per role.
  // Bump this number as each nation lands.
  if (batsmen.length !== 141) {
    throw new Error(`Expected 141 batsmen, got ${batsmen.length}`);
  }
  if (bowlers.length !== 141) {
    throw new Error(`Expected 141 bowlers, got ${bowlers.length}`);
  }
