...
function main(): void {
  ...
  // Currently: 7 nations expanded → 13×7 + 11×5 + 5 = 151 per role.
  // Bump this number as each nation lands.
  if (batsmen.length !== 151) {
    throw new Error(`Expected 151 batsmen, got ${batsmen.length}`);
  }
  if (bowlers.length !== 151) {
    throw new Error(`Expected 151 bowlers, got ${bowlers.length}`);
  }
